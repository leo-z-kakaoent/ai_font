...
class Discriminator(nn.Module):
    ...
    def __init__(self, D_ch=64, D_wide=True, resolution=128,
        ...
        # Architecture
        self.arch = D_arch(self.ch, self.attention, input_nc = input_nc)[resolution]
...
def content_encoder_arch(ch =16,out_channel_multiplier = 1, input_nc = 3):
    arch = {'in_channels':   [input_nc] + [ch*item for item in  [1,2,4,8]],
...
class ContentEncoder(nn.Module):
    ...
    def __init__(self, G_ch=64, G_wide=True, resolution=128,
        ...
        self.arch = content_encoder_arch(self.ch, self.out_channel_nultipiler,input_nc)
...
class Decoder(nn.Module):
    def __init__(self, G_ch=64, G_wide=True, resolution=128,
        ...
        # import pdb;pdb.set_trace()
        for index in range(len(self.arch['out_channels'])):
            upsample_function = functools.partial(F.interpolate, scale_factor=2,
