...
class LinearBlock(nn.Module):
    def __init__(self, in_dim, out_dim, norm='none', act='relu', use_sn=False):
        ...
        if norm == 'bn':
            ...
            self.norm = nn.InstanceNorm1d(norm_dim, affine=True, track_running_stats=True)
        elif norm == 'none':
            self.norm = None
        else:
...
class decoder_textedit_addskip(nn.Module):
    def __init__(self, G_ch=64, G_wide=True, resolution=128,
        ...
        self.blocks = nn.ModuleList([nn.ModuleList(block) for block in self.blocks])
        last_layer = nn.Sequential(nn.InstanceNorm2d(self.arch['out_channels'][-1], affine=True, track_running_stats=True),
                                        self.activation,
