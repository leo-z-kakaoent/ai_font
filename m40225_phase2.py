...
from font_diffuser.build import build_unet, build_style_encoder, build_content_encoder, build_scr, build_ddpm_scheduler
from font_diffuser.args import TrainPhase2Args
from font_diffuser.utils import save_args_to_yaml, x0_from_epsilon, reNormalize_img, normalize_mean_std

args = TrainPhase2Args()
set_seed(args.seed)
...
unet.load_state_dict(torch.load(args.unet_path))
style_encoder.load_state_dict(torch.load(args.style_encoder_path))
content_encoder.load_state_dict(torch.load(args.content_encoder_path))

scr = build_scr(args=args)
scr.load_state_dict(torch.load(args.scr_path))
...
train_font_dataset = FontDataset(path=args.path, scr=True, num_neg=args.num_neg)
